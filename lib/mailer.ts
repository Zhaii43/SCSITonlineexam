    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);">
        ...
        <tr><td style="background:#f8fafc;padding:16px 32px;text-align:center;font-size:12px;color:#94a3b8;">
          &copy; ${new Date().getFullYear()} SCSIT Online Exam &mdash; This is an automated message, please do not reply.
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export async function sendStudentApprovalEmail(
  ...
  await sendMail(
    ...
    `Hello ${firstName},\n\nCongratulations! Your SCSIT Online Exam student account has been approved.\n\nYou can now log in and start taking exams.\n\nLogin: ${frontendUrl}/login`,
    layout("Account Approved", `
...
export async function sendStudentRejectedEmail(
  ...
  await sendMail(
    ...
    `Hello ${firstName},\n\nUnfortunately your SCSIT Online Exam registration was not approved.\n\nReason: ${rejectionReason || "No reason provided."}\n\nContact support: ${frontendUrl}/contact`,
    layout("Registration Not Approved", `
      ...
      ${rejectionReason ? `<div style="background:#fef2f2;border-left:4px solid #ef4444;padding:12px 16px;border-radius:4px;margin:16px 0;"><strong>Reason:</strong> ${rejectionReason}</div>` : ""}
      <p>If you believe this is a mistake, please contact your department administrator.</p>
      <p style="margin-top:24px;">${btn(`${frontendUrl}/contact`, "Contact Support", true)}</p>
    `)
  );
}
...
export async function sendDeanExamCreatedEmail(
  to: string,
  fullName: string,
  exam: {
    id: number;
  ...
  await sendMail(
    ...
    layout(`Exam Created: ${exam.title}`, `
      <h2 style="margin:0 0 8px;">✅ Exam Created Successfully</h2>
      <p>Hello <strong>${fullName}</strong>,</p>
      <p>Your exam has been created and <strong>automatically approved</strong> (dean-created exams skip the review queue). You can now add questions.</p>
      <table cellpadding="0" cellspacing="0" style="margin:20px 0;width:100%;">
        ${detailRow("Exam Title", exam.title)}
        ${detailRow("Subject", exam.subject)}
        ${detailRow("Department", exam.department)}
...
export async function sendResultsPublishedEmail(
  ...
  await sendMail(
    ...
    layout(`Results: ${result.examTitle}`, `
...
export async function sendBulkImportEmail(
  to: string,
  firstName: string,
...
export async function sendAnnouncementEmail(
  to: string,
  firstName: string,
  announcement: { title: string; message: string; createdBy: string; createdAt: string },
  frontendUrl: string
) {
  await sendMail(
    to,
    `New Announcement: ${announcement.title} – SCSIT Online Exam`,
    `Hello ${firstName},\n\n${announcement.title}\n\n${announcement.message}\n\nPosted by: ${announcement.createdBy}\nDate: ${announcement.createdAt}\n\nView: ${frontendUrl}/dashboard`,
    layout(`Announcement: ${announcement.title}`, `
      ...
      <table cellpadding="0" cellspacing="0" style="margin:0 0 20px;width:100%;">
        ...
        ${detailRow("Date", announcement.createdAt)}
      </table>
      <p>${btn(`${frontendUrl}/dashboard`, "View Dashboard", true)}</p>
    `)
  );
}
...
export async function sendTimeExtensionEmail(
  ...
  await sendMail(
    ...
    layout(`Time Extended: ${exam.title}`, `
...
export async function sendExamRejectedEmail(
  to: string,
  firstName: string,
  examTitle: string,
  deanName: string,
  ...
  await sendMail(
    ...
    layout(`Exam Rejected: ${examTitle}`, `
...
export async function sendIssueReportEmail(
  ...
  const reportLink = role === "instructor"
  ...
  await sendMail(
    ...
    layout(`Issue Report: ${report.examTitle}`, `
...
export async function sendIssueReportReplyEmail(
  ...
  await sendMail(
    ...
    layout(`Issue Report Reply: ${report.examTitle}`, `
