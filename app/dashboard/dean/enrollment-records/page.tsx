...
const ADD_FORM_FIELDS: AddFormField[] = [
  { label: "School ID *", key: "school_id", placeholder: "e.g. 2024-001" },
...
export default function EnrollmentRecordsPage() {
  ...
  const [addForm, setAddForm] = useState<AddFormState>({ school_id: "", first_name: "", last_name: "", year_level: "1", email: "", contact_number: "" });
  ...
  const fetchRecords = useCallback(async (token: string, q = "") => {
    const url = q ? `${API_URL}/enrolled-records/?search=${encodeURIComponent(q)}` : `${API_URL}/enrolled-records/`;
  ...
  const fetchStudentStatuses = useCallback(async (token: string) => {
    ...
    ((data.students as StudentStatusRecord[] | undefined) || []).forEach((s) => {
      if (s.school_id) {
        map[String(s.school_id)] = {
  ...
  const refreshEnrollmentData = useCallback(async (token: string, q = searchRef.current) => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/profile/`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(p => {
        if (p.role !== "dean") { router.push("/dashboard"); return null; }
        return refreshEnrollmentData(token, "");
      })
      .then(() => setLoading(false))
      .catch(() => { setError("Failed to load enrollment records."); setLoading(false); });
  }, [refreshEnrollmentData, router]);

  useEffect(() => {
    searchRef.current = search;
    const token = localStorage.getItem("access_token");
    if (!token) return;
    const t = setTimeout(() => fetchRecords(token, search), 300);
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      const token = localStorage.getItem("access_token");
      if (token) refreshEnrollmentData(token);
    }, 15000);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { showToast("error", data.error || "Failed to add record"); return; }
      showToast("success", "Enrollment record added successfully");
      setAddForm({ school_id: "", first_name: "", last_name: "", year_level: "1", email: "", contact_number: "" });
      setShowAddForm(false);
      await fetchRecords(token, search);
    } finally {
      setAddLoading(false);
    }
  };
  ...
  const handleDownloadTemplate = async () => {
    const token = localStorage.getItem("access_token")!;
    const res = await fetch(`${API_URL}/enrolled-records/template/`, { headers: { Authorization: `Bearer ${token}` } });
    if (res.ok) {
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = "enrolled_students_template.csv"; a.click();
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (res.ok && data.success_count > 0) {
        await fetchRecords(token, search);
        showToast("success", `${data.success_count} records imported`);
      }
    } finally {
  ...
  return (
    <div className="min-h-screen bg-sky-200">
      ...
      {toast && (
        <div className={`fixed top-5 right-5 z-50 px-5 py-3 rounded-xl shadow-xl text-sm font-semibold text-white ${toast.type === "success" ? "bg-emerald-600" : "bg-red-600"}`}>
          {toast.text}
        </div>
      )}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
              ...
              <div className="mb-8 rounded-3xl bg-white/90 backdrop-blur-xl border border-slate-200 shadow-xl p-6 flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <h1 className="mt-1 text-2xl font-semibold text-slate-900">Enrollment Records</h1>
                  <p className="text-sm text-slate-500 mt-0.5">{records.length} official record{records.length !== 1 ? "s" : ""} — used to verify student registrations</p>
                </div>
              ...
              {showAddForm && (
                <div className="mb-6 bg-white/90 rounded-2xl border border-sky-200 shadow-lg p-6">
                  <h3 className="font-bold text-slate-900 mb-4">Add Enrollment Record</h3>
                  <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={e => setAddForm(p => ({ ...p, year_level: e.target.value }))}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-400"
                      >
                    ...
                    <div className="md:col-span-3 flex gap-3">
                      <button type="submit" disabled={addLoading} className="px-6 py-2.5 bg-slate-900 text-white rounded-xl font-semibold text-sm hover:bg-slate-800 disabled:opacity-50">
                        {addLoading ? "Saving..." : "Save Record"}
                      </button>
                      <button type="button" onClick={() => setShowAddForm(false)} className="px-6 py-2.5 bg-white border border-slate-300 text-slate-700 rounded-xl font-semibold text-sm hover:bg-slate-50">
                        Cancel
                      </button>
                    </div>
                  </form>
              ...
              {records.length === 0 ? (
                <div className="bg-white/90 rounded-3xl p-12 border border-slate-200 shadow-lg text-center">
                  ...
                  <h3 className="text-lg font-bold text-slate-900 mb-1">{search ? "No results found" : "No enrollment records yet"}</h3>
                  <p className="text-slate-500 text-sm">{search ? "Try a different search term." : "Add records manually or import from CSV."}</p>
                </div>
              ) : (
              <div className="rounded-2xl border border-slate-200 bg-white/90 shadow-xl overflow-hidden">
                  <div className="hidden md:grid grid-cols-[1.1fr_1.6fr_1fr_1.1fr_1.5fr_1fr_80px] gap-3 px-5 py-3 text-[11px] font-semibold uppercase tracking-wide text-slate-500 bg-slate-50 border-b border-slate-200">
                    <span>School ID</span>
                    <span>Name</span>
                  ...
                  <div className="divide-y divide-slate-100">
                    {records.map(r => (
                      <div key={r.id} className="grid grid-cols-1 md:grid-cols-[1.1fr_1.6fr_1fr_1.1fr_1.5fr_1fr_80px] gap-3 px-5 py-3 items-center hover:bg-slate-50 transition-colors">
                        <p className="text-sm font-semibold text-slate-900 font-mono">{r.school_id}</p>
                        <p className="text-sm text-slate-800">{r.first_name} {r.last_name}</p>
                        <span className="inline-flex w-fit px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-purple-100 text-purple-700">{YEAR_LABELS[r.year_level] || r.year_level}</span>
                        <div className="flex flex-wrap gap-2">
                          {statusMap[String(r.school_id)]?.is_transferee && (
                            <span className="inline-flex w-fit px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-sky-100 text-sky-700">Transferee</span>
                          )}
                          {statusMap[String(r.school_id)]?.is_irregular && (
                            <span className="inline-flex w-fit px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-amber-100 text-amber-700">Irregular</span>
                          )}
                          {!statusMap[String(r.school_id)]?.is_transferee && !statusMap[String(r.school_id)]?.is_irregular && (
                            <span className="text-xs text-slate-400">—</span>
                          )}
                        </div>
                        <p className="text-xs text-slate-500 truncate">{r.email || "—"}</p>
                        <p className="text-xs text-slate-500">{r.contact_number || "—"}</p>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl border border-slate-200">
              <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                <h2 className="text-lg font-bold text-slate-900">Import Enrollment Records</h2>
                <button onClick={() => { setShowImportModal(false); setCsvFile(null); setImportResult(null); }} className="text-slate-400 hover:text-slate-600 text-2xl leading-none">×</button>
              </div>
              <div className="p-6 space-y-4">
                <div className="bg-slate-50 rounded-xl p-4 text-sm text-slate-600 space-y-1">
                  ...
                  <p className="font-mono text-xs">school_id, first_name, last_name, year_level</p>
                  <p className="font-semibold text-slate-800 mt-2">Optional columns:</p>
                  <p className="font-mono text-xs">email, contact_number</p>
                  <p className="text-xs text-slate-500 mt-1">year_level accepts: 1st, 2nd, 3rd, 4th (or 1, 2, 3, 4)</p>
                </div>
                <div>
                ...
                {importResult && (
                  <div className={`rounded-xl p-4 text-sm ${importResult.error ? "bg-red-50 border border-red-200 text-red-700" : "bg-emerald-50 border border-emerald-200 text-emerald-800"}`}>
                    {importResult.error ? (
                      <p>{importResult.error}</p>
                    ) : (
                      <>
                        <p className="font-semibold">{importResult.success_count} records imported, {importResult.error_count} errors</p>
                        {(importResult.errors ?? []).length > 0 && (
                          <div className="mt-2 max-h-40 overflow-y-auto space-y-1">
                            {(importResult.errors ?? []).map((e, i: number) => (
